...
function initializeForm() {
    ...
    if (savedData) {
        const shouldRestore = confirm('يوجد نموذج محفوظ مسبقاً. هل تريد استعادته؟\nThere is a previously saved form. Would you like to restore it?');
        if (shouldRestore) {
...
function setupEventListeners() {
    ...
    form.addEventListener('reset', function(e) {
        setTimeout(() => {
            ...
            showNotification('تم مسح النموذج بنجاح / Form cleared successfully', 'info');
        }, 100);
    });
}
...
function printForm() {
    ...
    if (!validateForm(false)) {
        const proceed = confirm('بعض الحقول فارغة. هل تريد الطباعة على أي حال؟\nSome fields are empty. Do you want to print anyway?');
        if (!proceed) return;
    }
...
function saveForm() {
    ...
    const downloadOption = confirm('هل تريد تحميل نسخة من النموذج؟\nWould you like to download a copy of the form?');
    if (downloadOption) {
...
function loadFormData(data) {
    ...
    showNotification('تم استعادة النموذج بنجاح / Form restored successfully', 'success');
}
...
function validateForm(showErrors) {
    const requiredFields = [
        { id: 'contractNo', name: 'رقم العقد / Contract No.' },
        { id: 'customerName', name: 'اسم العميل / Customer Name' },
        { id: 'make', name: 'نوع المركبة / Make' },
        { id: 'model', name: 'الموديل / Model' },
        { id: 'year', name: 'سنة الصنع / Year' },
        { id: 'vin', name: 'رقم الهيكل / VIN' }
    ];
...
function previewImage(input, previewId) {
    ...
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
            preview.innerHTML = `<img src="${e.target.result}" alt="صورة المركبة">`;
            if (removeBtn) {
...
function exportToExcel() {
    // Validate form first
    if (!validateForm(false)) {
        const proceed = confirm('بعض الحقول فارغة. هل تريد التصدير على أي حال؟\nSome fields are empty. Do you want to export anyway?');
        if (!proceed) return;
    }
    
    showLoadingOverlay('جاري إنشاء ملف Excel... / Creating Excel file...');
    ...
    try {
        ...
        hideLoadingOverlay();
        showNotification('تم تصدير ملف Excel بنجاح! / Excel file exported successfully!', 'success');
        
    } catch (error) {
        console.error('Excel Export Error:', error);
        hideLoadingOverlay();
        showNotification('حدث خطأ أثناء إنشاء ملف Excel.\nAn error occurred while creating Excel file.', 'error');
    }
}
...
async function exportToPDF() {
    ...
    showLoadingOverlay('جاري إنشاء ملف PDF... / Creating PDF...');
    ...
    try {
        ...
        hideLoadingOverlay();
        showNotification('تم تصدير ملف PDF بنجاح! / PDF exported successfully!', 'success');
...
function clearSavedData() {
    const confirmDelete = window.confirm('هل أنت متأكد من حذف البيانات المحفوظة؟\nAre you sure you want to delete saved data?');
    if (confirmDelete) {
        localStorage.removeItem('vehicleEvaluationForm');
        showNotification('تم حذف البيانات المحفوظة / Saved data deleted', 'info');
    }
}
